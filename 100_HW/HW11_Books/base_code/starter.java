...
class starter {
		...
		*/
		while(true){
			...
			if(choice.equals("file")){
				...
				int c = checksArray(arr, b);
				System.out.println(b + " was said " + c + " times");
			}
			
			else if(choice.equals("common")){
				System.out.println("The most used word was " + mostCommon(arr));
			}
	...
	*/
	public static void printsNumber(String[] arr, int num){
		...
		if(num>arr.length){
			System.out.print("The number is too large, try again");
		}
		else{
			for(int i = 0; i<num; i++){
				System.out.print(arr[i] + " ");
